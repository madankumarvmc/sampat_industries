  ...
  validate: function (frm) {
    ...
    function addEmployeeCheckinArrayFunction(addEmployeeCheckinArray) {
      frappe
        .call({
          method:
            "sampat_industries.sampat_industries.doctype.mark_daily_attendance.mark_daily_attendance.add_to_employee_checkin",
    ...
    function updateEmployeeCheckinArrayFunction(updateEmployeeCheckinArray) {
      frappe
        .call({
          method:
            "sampat_industries.sampat_industries.doctype.mark_daily_attendance.mark_daily_attendance.update_to_employee_checkin",
    ...
    function markAttendance(employee, date, company, status) {
      frappe
        .call({
          method:
            "sampat_industries.sampat_industries.doctype.mark_daily_attendance.mark_daily_attendance.add_to_attendance",
    ...
    function addEmployeeAdvance(employee, date, advanceAmt) {
      frappe
        .call({
          method:
            "sampat_industries.sampat_industries.doctype.mark_daily_attendance.mark_daily_attendance.add_to_employee_advance",
    ...
    function deleteEmployeeCheckinDoc(
      absent,
      login_checkinid,
      logout_checkinid
    ) {
      if (
        absent == "1" &&
        (login_checkinid != null || logout_checkinid != null)
      ) {
        ...
        if (login_checkinid != null) {
          frappe
            .call({
              method:
                "sampat_industries.sampat_industries.doctype.mark_daily_attendance.mark_daily_attendance.delete_from_employee_checkin",
    ...
    function pushUpdateEmployeeCheckinfunction(
      employee,
      time,
      loginType,
      checkinid,
      ...
      updateEmployeeCheckinArray.push({
        employee: employee,
        time: time,
        loginType: loginType,
        checkinid: checkinid,
    ...
    function pushAddEmployeeCheckinFunction(
      employee,
      time,
      loginType,
      checkinid,
    ...
    data.forEach((e) => {
      if (e.in_time) {
        ...
        if (e.login_checkinid) {
          ...
          pushUpdateEmployeeCheckinfunction(
            e.employee,
            e.in_time,
            "IN",
            e.login_checkinid,
          ...
          pushAddEmployeeCheckinFunction(
            e.employee,
            e.in_time,
            "IN",
            e.login_checkinid,
...
frappe.ui.form.on("Daily Attendance Child", {
  ...
  absent: function (frm, cdt, cdn) {
    ...
    const absent = child.absent;
    const approved = child.approved;
...
function fetchEmployeeAttendance(frm) {
  ...
  frappe
    .call({
      method:
        "sampat_industries.sampat_industries.doctype.mark_daily_attendance.mark_daily_attendance.get_employees_checkin",
      args: { date, company },
    })
    .done((r) => {
      ...
      $.each(r.message, function (_i, e) {
        ...
        if (e.attendance == "Absent") {
          ...
          entry.employee_name =
            "<span style='color:red'>" + e.employee_name + "</span>";
        } else {
        ...
        if (total_hrs <= 0) {
          ...
          entry.total_hrs =
            "<span style='color:orange'>" + total_hrs_toupdate + "</span>";
        } else {
