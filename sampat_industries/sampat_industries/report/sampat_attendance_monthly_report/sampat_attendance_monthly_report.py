...
def get_columns(filters: Filters) -> List[Dict]:
	...
	if filters.summarized_view:
		...
		columns.append({"label": _("Total Present"),"fieldname": "total_present_days","fieldtype": "Float","width": 110})
...
def get_employee_related_details(filters: Filters) -> Tuple[Dict, List]:
	...
	query = (
		...
		.select(
			...
			Employee.branch,
			Employee.company,
   			Employee.esi_company,
			Employee.holiday_list,
		)
		.where((Employee.esi_company == filters.esi_company) | (Employee.company == filters.company))
	)
...
def get_rows(
	...
	for employee, details in employee_details.items():
		...
		if filters.summarized_view:
			...
			total_present = attendance.get('total_present', 0)
			attendance_for_employee[0].update(
				{"employee": employee, "employee_name": details.employee_name, "total_present_days":total_present}
			)
